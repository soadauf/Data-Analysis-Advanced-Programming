...
setwd("D:/Uni 4/advanced/project")  # Replace with your folder path
getwd()
# Load the data
table1 <- read.table("GSE188847.top.table.tsv", sep = "\t", header = TRUE, quote = "", fill = TRUE)
table2 <- read.table("GSE104704.top.table.tsv", sep = "\t", header = TRUE, quote = "", fill = TRUE)
...
ggplot(merged_table, aes(x = log2FC_1, y = -log10(padj_1))) +
  ...
  scale_color_manual(values = c("Upregulated" = "pink", "Downregulated" = "cyan", "Neutral" = "gray")) +
  labs(title = "Volcano Plot", x = "Log2 Fold Change", y = "-Log10(Adjusted P-value)", color = "Gene Status") +
  theme_minimal()
